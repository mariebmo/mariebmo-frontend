<script lang="ts">
	...
	function increase() {
        ...
        if(totalAmountIncluded && current > amount){
            visualizationOutput = "You can't increase from " + current + " to " + amount + " stitches, silly! Did you mean to decrease or increase by " + amount + " stitches?";
            knittingLingoOutput = '';
            return;
        }

        if(totalAmountIncluded && (amount - current) > current || !totalAmountIncluded && amount > current ){
            visualizationOutput = "This calculator doesn't support increasing by more than the current amount of stitches."
            knittingLingoOutput = '';
            return;
        }
...
<div id='knitting-page'>
    ...
    <div id='knitting-calculator-search'>
        ...
        <input id="current" class="input-number calculator-element" type="number" bind:value={current} />
    
        <button on:click={toggleIncreaseDecrease} id='increase-decrease-btn' class='calculator-element'>{increaseOrDecrease}</button>
        <button on:click={toggleByOrTo} id='by-to-btn' class="calculator-element">{byOrTo}</button>
    
        <input id="amount" class="input-number calculator-element" type="number" bind:value={amount} />
